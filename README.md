# Building TIGER 2023 Road Tiles
...
1. Download TIGER data. `ROADS` for the geometries, `FEATNAMES` for the split-apart road names.
   ...
   curl -s https://www2.census.gov/geo/tiger/TIGER2023/FEATNAMES/ | \
      grep -o '<a href=['"'"'"][^"'"'"']*['"'"'"]' | \
      grep featnames | \
      sed -e 's/^<a href=["'"'"']/https:\/\/www2.census.gov\/geo\/tiger\/TIGER2023\/FEATNAMES\//' -e 's/["'"'"']$//' | \
      xargs -I {} -P 24 -n 1 sh -c 'export f={}; curl -s -o /mnt/tiger/featnames/$(basename $f) $f; echo $f'

   mkdir -p /mnt/tiger/roads
   curl -s https://www2.census.gov/geo/tiger/TIGER2023/ROADS/ | \
      grep -o '<a href=['"'"'"][^"'"'"']*['"'"'"]' | \
      grep roads | \
      sed -e 's/^<a href=["'"'"']/https:\/\/www2.census.gov\/geo\/tiger\/TIGER2023\/ROADS\//' -e 's/["'"'"']$//' | \
...
1. Save the mbtiles file to S3 so you don't have to do this again.
   ...
   aws s3 cp \
      ...
      s3://data.openstreetmap.us/tiger2023_expanded_roads.mbtiles
   ```
