# Building TIGER 2024 Road Tiles
...
1. Start an EC2 instance with multiple vCPUs. I used an `m8g.2xlarge`. Use Ubuntu 24.04 and set up the security group so you can SSH to the instance.
...
1. Install build and compile dependencies:
   ...
   ```
   sudo apt-get -y install build-essential libsqlite3-dev curl
   ```
   
1. Download, compile, and install the proj source:

   ```
   cd /tmp
   curl -L http://download.osgeo.org/proj/proj-9.6.0.tar.gz | tar zxf -
   cd proj-9.6.0/
   mkdir build
   cd build
   cmake -DENABLE_TIFF=OFF -DENABLE_CURL=OFF -DBUILD_PROJSYNC=OFF -DCMAKE_INSTALL_PREFIX=/usr/local ..
   cmake --build . -j $(nproc)
...
1. Download, compile, and install the gdal source:
   ...
   ```
   cd /tmp
   curl -L https://download.osgeo.org/gdal/3.9.3/gdal-3.9.3.tar.gz | tar zxf -
   cd gdal-3.9.3/
   mkdir build
   cd build
   ...
   Add the installed libraries to the library path: 
...
1. Download, compile, and install the libgeos sources:
   ...
   ```
   cd /tmp
   curl -L https://github.com/libgeos/geos/archive/3.13.1.tar.gz | tar zxf -
   cd geos-3.13.1/
   mkdir build
   cd build
   cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local ..
...
1. Download, compile, and install the Tippecanoe source:
   ...
   ```
   cd /tmp
   curl -L https://github.com/felt/tippecanoe/archive/2.75.1.tar.gz | tar zxf -
   cd tippecanoe-2.75.1/
   make -j$(nproc)
   sudo make install
   ```
...
1. Download TIGER data. `ROADS` for the geometries, `FEATNAMES` for the split-apart road names.
   ...
   curl -s https://www2.census.gov/geo/tiger/TIGER2024/FEATNAMES/ | \
      grep -o '<a href=['"'"'"][^"'"'"']*['"'"'"]' | \
      grep featnames | \
      sed -e 's/^<a href=["'"'"']/https:\/\/www2.census.gov\/geo\/tiger\/TIGER2024\/FEATNAMES\//' -e 's/["'"'"']$//' | \
      xargs -I {} -P 24 -n 1 sh -c 'export f={}; curl -s -o /mnt/tiger/featnames/$(basename $f) $f; echo $f'

   mkdir -p /mnt/tiger/roads
   curl -s https://www2.census.gov/geo/tiger/TIGER2024/ROADS/ | \
      grep -o '<a href=['"'"'"][^"'"'"']*['"'"'"]' | \
      grep roads | \
      sed -e 's/^<a href=["'"'"']/https:\/\/www2.census.gov\/geo\/tiger\/TIGER2024\/ROADS\//' -e 's/["'"'"']$//' | \
...
1. Save the mbtiles file to S3 so you don't have to do this again.
   ...
   aws s3 cp \
      ...
      s3://data.openstreetmap.us/tiger2024_expanded_roads.mbtiles
   ```
